...
namespace SIL.Tests.ClearShare
{
	...
	public class MetadataTests
	{
		...
		{
			_outgoing.License = new CustomLicenseBase() { RightsStatement = "Use this if you must." };
			_outgoing.Write();
			var license = (CustomLicenseBase)Metadata.FromFile(_tempFile.Path).License;
		...
		{
			_outgoing.License = new CreativeCommonsLicenseBase(false, true, CreativeCommonsLicenseBase.DerivativeRules.Derivatives);
			_outgoing.Write();
			var cc = (CreativeCommonsLicenseBase)Metadata.FromFile(_tempFile.Path).License;
			Assert.AreEqual(cc.AttributionRequired, false);
			Assert.AreEqual(cc.CommercialUseAllowed, true);
			Assert.AreEqual(cc.DerivativeRule, CreativeCommonsLicenseBase.DerivativeRules.Derivatives);
		}
		...
		public void RoundTripPng_ManyProperties_RecoversAll()
		{
			...
			var cc = (CreativeCommonsLicenseBase)incoming.License;
			Assert.That(cc.AttributionRequired, Is.False);
			Assert.That(cc.CommercialUseAllowed, Is.True);
			Assert.That(cc.DerivativeRule, Is.EqualTo(CreativeCommonsLicenseBase.DerivativeRules.Derivatives));
			Assert.That(cc.RightsStatement, Is.EqualTo("Please attribute nicely"));
		...
		public void CanRemoveRightsStatment()
		{
			...
			intermediate.License = new CreativeCommonsLicenseBase(false, true,
				CreativeCommonsLicenseBase.DerivativeRules.Derivatives); // no rights
			intermediate.Write();
			var incoming = Metadata.FromFile(_tempFile.Path);
			var cc = (CreativeCommonsLicenseBase)incoming.License;
			Assert.That(cc.AttributionRequired, Is.False);
		...
		{
			_outgoing.License = new CreativeCommonsLicenseBase(true, false, CreativeCommonsLicenseBase.DerivativeRules.NoDerivatives);
			_outgoing.Write();
			var cc = (CreativeCommonsLicenseBase)Metadata.FromFile(_tempFile.Path).License;
			Assert.AreEqual(cc.AttributionRequired, true);
			Assert.AreEqual(cc.CommercialUseAllowed, false);
			Assert.AreEqual(cc.DerivativeRule, CreativeCommonsLicenseBase.DerivativeRules.NoDerivatives);
		}
		...
		{
			_outgoing.License = new CreativeCommonsLicenseBase(true, true, CreativeCommonsLicenseBase.DerivativeRules.DerivativesWithShareAndShareAlike);
			_outgoing.Write();
			var cc = (CreativeCommonsLicenseBase)Metadata.FromFile(_tempFile.Path).License;
			Assert.AreEqual(cc.AttributionRequired, true);
			Assert.AreEqual(cc.CommercialUseAllowed, true);
			Assert.AreEqual(cc.DerivativeRule, CreativeCommonsLicenseBase.DerivativeRules.DerivativesWithShareAndShareAlike);
		}
		[Test]
		...
		public void SetLicense_HasChanges_True()
		{
			var m = new Metadata();
			m.HasChanges = false;
			m.License = new CreativeCommonsLicenseBase(true, true, CreativeCommonsLicenseBase.DerivativeRules.Derivatives);
		...
		public void ChangeLicenseDetails_HasChanges_True()
		{
			...
			m.License = new CreativeCommonsLicenseBase(true, true, CreativeCommonsLicenseBase.DerivativeRules.Derivatives);
			m.HasChanges = false;
			((CreativeCommonsLicenseBase)m.License).CommercialUseAllowed = false;
			Assert.IsTrue(m.HasChanges);
		}
		...
		public void LoadXmpFile_ValuesCopiedFromOtherFile()
		{
			...
			original.License = new CreativeCommonsLicenseBase(true, true, CreativeCommonsLicenseBase.DerivativeRules.Derivatives);
		...
		{
			var m = new Metadata();
			m.License = new CreativeCommonsLicenseBase(true, true,
												   CreativeCommonsLicenseBase.DerivativeRules.
		...
		public void MinimalCredits_CreativeCommonsNoCreator()
		{
			var m = new Metadata
			{
				CopyrightNotice = "Copyright © 2014 SIL",
				CollectionName = "My Collection",
				License = new CreativeCommonsLicenseBase(true, true, CreativeCommonsLicenseBase.DerivativeRules.DerivativesWithShareAndShareAlike)
				{
