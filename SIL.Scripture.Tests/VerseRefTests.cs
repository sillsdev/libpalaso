...
namespace SIL.Scripture.Tests
{
    ...
    public class VerseRefTests
    {
        ...
        {
            XmlSerializer sr = new XmlSerializer(typeof(VerseRef));
            using (MemoryStream ms = new MemoryStream())
            {
                sr.Serialize(ms, new VerseRef("LEV", "12", "6-7a", ScrVers.Vulgate));

                ms.Position = 0;
                
                VerseRef vRefRestored = sr.Deserialize(ms) as VerseRef;
                Assert.IsNotNull(vRefRestored);
                Assert.AreEqual("LEV", vRefRestored.Book);
                Assert.AreEqual("12", vRefRestored.Chapter);
                Assert.AreEqual("6-7a", vRefRestored.Verse);
                Assert.AreEqual(ScrVers.Vulgate, vRefRestored.Versification);
            }
        }
        #endregion
