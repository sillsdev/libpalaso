...
namespace SIL.Media.Tests
{
	...
	public class AudioSessionTests
	{
		...
		/// <summary>
		/// Tests that when attempting to play a file whose contents are not a valid audio file, the error is reported as a non-fatal error.
		/// </summary>
		[Test]
		...
		public void Play_InvalidAudioFileThrowsBackgroundException_NonFatalErrorReported()
		{
			try
			{
				...
				session.PlaybackStopped += (sender, args) =>
				{
					reportedExceptionInPlaybackStopped = ((StoppedEventArgs)args).Exception;
				};
				Assert.DoesNotThrow(() => session.Play(),
			...
			{
				AudioFactory.PlaybackErrorMessage = null;
			}
		}
#endif
