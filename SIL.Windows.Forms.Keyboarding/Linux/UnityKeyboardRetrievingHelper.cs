...
namespace SIL.Windows.Forms.Keyboarding.Linux
{
	...
	internal class UnityKeyboardRetrievingHelper
	{
		...
		private static string[] GetMyKeyboards()
		{
			...
			if (list.Length < 1 && Platform.IsFlatpak)
			{
				// Querying gsettings from within flatpak needs to go thru a flatpak portal.
				list = GnomeInputSourcesViaFlatpakPortal();
			}
		...
		/// For example, parses "(<<[('xkb', 'us'), ('ibus', 'libpinyin')]>>,)\n" to
		/// the list { "xkb;;us", "ibus;;libpinyin" }.
		/// </summary>
		internal static List<string> ParseGDBusKeyboardList(string keyboardList)
		{
			string[] keyboards = keyboardList.Split(new string[] {"), "}, StringSplitOptions.RemoveEmptyEntries);
			...
			foreach(var keyboard in keyboards) {
				string[] keyboardDesignationTuple = keyboard.Trim("(<[']>),\n".ToCharArray()).Split(new string[] {"', '"}, StringSplitOptions.RemoveEmptyEntries);
