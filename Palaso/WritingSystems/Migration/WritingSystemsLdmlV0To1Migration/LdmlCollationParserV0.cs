	...
	public class LdmlCollationParserV0
	{
		...
		public static string GetIcuRulesFromCollationNode(string collationXml)
		{
			...
			using (XmlReader collationReader = XmlReader.Create(new StringReader(collationXml), readerSettings))
			{
				...
				if (XmlHelpersV0.FindNextElementInSequence(collationReader, "suppress_contractions", LdmlNodeComparer.CompareElementNames))
				{
					icuRules += GetIcuOptionFromNode(collationReader);
				}
				if (XmlHelpersV0.FindNextElementInSequence(collationReader, "optimize", LdmlNodeComparer.CompareElementNames))
				{
					icuRules += GetIcuOptionFromNode(collationReader);
				}
