...
/*
		...
		*/
		...
		{
			var OSVersion = ErrorReport.GetOperatingSystemLabel();
			...
			if (DateTime.UtcNow.Date != _settings.PreviousLaunchDate.Date)
			{
				SendNavigationNotice("{0}/launch/version{1}", OSVersion, ErrorReport.VersionNumberString);
			}
			...
			{
				SendNavigationNotice("{0}/firstApparentLaunchForAnyVersionOnMachine" + "/" + ErrorReport.VersionNumberString, OSVersion);
			}
			else if (_realPreviousVersion != ErrorReport.VersionNumberString)
			{
				SendNavigationNotice("{0}/versionChange/version{1}-previousVersion{2}", OSVersion, ErrorReport.VersionNumberString, _realPreviousVersion);
			}
		}
