...
namespace Palaso.IO
{
	public static class PathUtilities
	{
		...
		public static bool DeleteToRecycleBin(string filePath)
		{
			...
			// and http://www.ramendik.ru/docs/trashspec.html.
			...
			if (Directory.Exists(filePath) || File.Exists(filePath))
			{
				...
				var trashFilesPath = Path.Combine(trashPath, "files");
				var trashedFileName = Path.GetFileName(filePath);

				var recyclePath = Path.Combine(trashFilesPath, trashedFileName);
				if (Directory.Exists(recyclePath) || File.Exists(recyclePath))
				{
					trashedFileName = Path.GetRandomFileName();
					recyclePath = Path.Combine(trashFilesPath, trashedFileName);
					if (Directory.Exists(recyclePath) || File.Exists(recyclePath))
						return false;
				}
