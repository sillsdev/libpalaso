...
namespace SIL.Media
{
	...
	public class FFmpegRunner
	{
		...
		private static string LocateFFMpeg()
		{
			...
			if (!IsNullOrEmpty(withApplicationDirectory) && File.Exists(withApplicationDirectory))
		...
		///</summary>
		public static bool HaveNecessaryComponents => !IsNullOrEmpty(LocateFFMpeg());
			...
			string outputPath, int channels, IProgress progress)
		{
			...
			var arguments = Format("-i \"{0}\" -vn -ac {1} \"{2}\"",
		...
		private static ExecutionResult RunFFMpeg(string arguments, IProgress progress)
		{
			...
			return CommandLineRunner.Run(LocateAndRememberFFMpeg(), arguments,
