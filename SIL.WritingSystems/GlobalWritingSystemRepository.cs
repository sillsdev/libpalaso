	...
	public abstract class GlobalWritingSystemRepository<T> : WritingSystemRepositoryBase<T>, IDisposable where T : WritingSystemDefinition
	{
		...
		private class WsStasher : IDisposable
		{
			...
			public void Dispose()
			{
				if (!_hasStash)
					return;
				// base.Replace removes the writing system before adding it back, so _wsFile is
				// normally gone by now. Overwrite rather than rely on that ordering.
				RobustFile.Move(_stashFile, _wsFile, overWrite: true);
				_hasStash = false;
			}
		}
	}
