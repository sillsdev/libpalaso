...
namespace SIL.WritingSystems
{
	...
	public class LanguageLookup
	{
		...
		{
			// Find the best name to use as "DesiredName".  This matches what gets stored
			// eventually as Names[0].  (This could be viewed as redundant since DesiredName
			// returns Names[0] if it is not set explicitly.)
			string desiredName = name;
			if (desiredName == null)
			{
				desiredName = localName;
				if (desiredName == null)
				{
			...
			// If we end up needing to add the language code (in the guise of desiredName), that reflects
			// a deficiency in the data.  But having a bogus name value probably hurts less that not having
			// any name at all.  The sort process mentioned above using the language tag as well as the first
			// two items in the Names list.
			if (language.Names.Count == 0)
				language.Names.Add(desiredName);
