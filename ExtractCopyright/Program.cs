...
namespace SIL.ExtractCopyright
{
	class Program
	{
		...
		static int ShowUsage(CopyrightFile.ExitValue retval)
		{
			...
			Console.WriteLine("acknowledgements embedded in the .Net assemblies.");
			Console.WriteLine("Usage: ExtractCopyright [options]");
			Console.WriteLine("-d (--debian)   provide the path to the debian folder");
			Console.WriteLine("-h (--help)     display this message");
			Console.WriteLine("-p (--prefix)   provide the directory prefix to add to DLL filenames in copyright file");
			return (int)retval;
		}
	}
