...
namespace Palaso.Tests.IO
{
	...
	public class PathUtilitiesTests
	{
		...
		public void DeleteToRecycleBin_MovedToTrashBin()
		{
			...
			var trashedFile = Path.Combine(trashBinPath, "files", Path.GetFileName(file));
			using (var tempTrashedFile = TempFile.TrackExisting(trashedFile))
			{
				Assert.That(File.Exists(trashedFile), Is.True);
				Assert.That(File.ReadAllText(trashedFile), Is.EqualTo(content));

				var metaFile = Path.Combine(trashBinPath, "info", Path.GetFileName(file) + ".trashinfo");
				using (var tempMetaFile = TempFile.TrackExisting(metaFile))
				{
