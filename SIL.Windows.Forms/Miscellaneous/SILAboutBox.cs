...
namespace SIL.Windows.Forms.Miscellaneous
{
	public partial class SILAboutBox : Form
	{
		...
		private void SILAboutBoxShown(object sender, EventArgs e)
		{
			...
			aboutBoxHtml = EnsureUtf8MetaTag(aboutBoxHtml);
		...
		private static string EnsureUtf8MetaTag(string newHtmlContents)
		{
			const string minimalHtmlContents = "<html><head><meta charset='UTF-8' /></head><body></body><ul>" +
			                                   DependencyMarker + "</ul></html>";
			// In order to correctly render even common things like copyright symbols,
			// we need a meta charset tag in the html header.
			XmlDocument dom;
			XmlNode headNode;
			try
			{
				dom = new XmlDocument {InnerXml = newHtmlContents};
				var charsetNode = dom.SelectSingleNode("//head/meta[@charset]");
				if (charsetNode != null)
				{
					return newHtmlContents; // don't overwrite any existing charset attribute
				}
