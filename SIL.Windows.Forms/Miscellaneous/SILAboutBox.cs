	...
	public partial class SILAboutBox : Form
	{
		...
		private void SILAboutBoxShown(object sender, EventArgs e)
		{
			...
			else
			{
				...
				// first try to create the temporary file in the same location we got the html file from
				try
				{
					var temporaryName = "temp." + Path.GetFileName(filePath);
					var tempPath = Path.GetDirectoryName(filePath) + Path.DirectorySeparatorChar + temporaryName;
					File.WriteAllText(tempPath, newHtmlContents);
					_tempAboutBoxHtmlFile = new TempFile(tempPath, true);
				...
				// if that doesn't work, use a "real" TempFile, but it's Temp directory location will make it hard
				// to find any linked css.
