...
namespace SIL.Windows.Forms.Miscellaneous
{
	public partial class SILAboutBox : Form
	{
		...
		private TempFile _tempAboutBoxHtml; // after update by AcknowledgementsProvider
		...
		private void SILAboutBoxShown(object sender, EventArgs e)
		{
			...
			else
			{
				...
				_tempAboutBoxHtml = new TempFile(newHtmlContents);
				_browser.Navigate(_tempAboutBoxHtml.Path);
			}
		}
		...
		protected override void OnClosed(EventArgs e)
		{
			try
			{
				File.Delete(_tempAboutBoxHtml.Path);
			}
			catch (Exception ex)
			{
				Debug.WriteLine("Temporary file deletion failed. " + _tempAboutBoxHtml.Path + ex.Message);
			}
			finally
			{
				base.OnClosed(e);
