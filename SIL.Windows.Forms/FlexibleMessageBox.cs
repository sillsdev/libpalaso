			...
			private static string GetButtonTextAsLocalizedByL10nSharp(ButtonID buttonId)
			{
				...
				switch (buttonId)
				{
					...
					case ButtonID.Abort: return LocalizationManager.GetString("Common.AbortButton", "&Abort",
						"This button is the first in the set [Abort, Retry, Ignore] used by different dialogs. It can mean either abort the current" +
						" task or close the entire program. The text of each dialog frequently explains what each button does.");
