<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/DBLMetadata">
        <xsl:choose>
            <xsl:when test="type/medium/text() != 'text' or not(@version) or not(starts-with(@version, '2.2'))">
    ...
    <xsl:template name="agencies-content">
        <xsl:for-each select="agencies/rightsHolder">
            <xsl:copy>
                ...
                <xsl:if test="nameLocal">
                    <xsl:attribute name="local"><xsl:value-of select="nameLocal/text()"/></xsl:attribute>
                </xsl:if>
