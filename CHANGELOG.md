...
- [SIL.WritingSystems] `GlobalWritingSystemRepository` and the SLDR cache now build a file's new contents alongside the old one and swap the two, instead of writing over the live file. Both stores are guarded by a machine-wide mutex that a dying process can abandon, so an interrupted write could leave a definition truncated or, in `GlobalWritingSystemRepository.SaveDefinition`, missing entirely; a truncated SLDR cache entry was then copied over the caller's own good LDML by `Sldr.GetLdmlFile`. Each replacement is built at a path unique to the call, so writers sharing a directory cannot collide on it, and a target that appears after it was found absent is replaced rather than throwing, which used to abandon every writing system still queued behind it. The previous contents now survive an interrupted write, and the file left in place carries the permissions of the replacement rather than whichever the file system happens to keep. Where a file system cannot replace one file with another and the target has to be copied over, the contents being displaced are kept beside it until the copy completes. A writing system generated from a template no longer reaches the live path before it is complete. In the SLDR cache, an approved entry now supersedes the uid-qualified entry it came from only once the replacement is in place, so an interrupted update can no longer leave the cache with neither, and `DownloadLanguageTags` records the ETag only after the downloaded tags are in place -- recording it first could mark a truncated file as current, and a present file whose ETag matches is never re-fetched. `Sldr.GetLdmlFile` also delivers the entry to the caller by building it beside their file and swapping it in, rather than copying straight onto it: the caller has no second copy of that file, so an interrupted copy left them with neither their own definition nor a complete one. The SLDR cache entry is also built under the shared store's mask, since the swap puts the replacement's permissions on the entry left in place and that cache is shared between users. Both SLDR cache writers are now written through rather than buffered, since the swap that follows is a rename and can complete before the data blocks reach the disk; whether they actually do is a property of the operating system and the disk, so it is not something a test here can assert.
